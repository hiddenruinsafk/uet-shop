...
const CATS = ["Tutti","Pittura","Scultura","Fotografia","Illustrazione","Ceramica","Gioielli","Tessile","Altro"];
const PER = 12;
...
/* ─── STORAGE (localStorage — funziona su qualsiasi hosting) ─── */
const db = {
  get(k, d = null) {
...
body::before{content:'';position:fixed;inset:0;background-image:url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.9' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='.04'/%3E%3C/svg%3E");pointer-events:none;z-index:9998;opacity:.7;}
...
const Header = ({ cartCount = 0, onCart, onLogo, onBack }) => (
  <header style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(246,243,238,.9)", backdropFilter: "blur(18px)", borderBottom: "1px solid var(--border)", display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", padding: "0 18px", height: 60 }}>
    ...
    <div onClick={onLogo} style={{ cursor: "pointer", textAlign: "center", userSelect: "none" }}>
      <span style={{ fontFamily: "var(--serif)", fontSize: 20, fontWeight: 300, letterSpacing: ".32em" }}>ATELIER</span>
    </div>
...
const ProductCard = ({ p, onClick, idx }) => (
...
function ShopView({ products, cat, onCat, onProduct, cartCount, onCart, onLogo, onMore, hasMore }) {
  ...
  return (
    <div style={{ minHeight: "100vh" }}>
      ...
      <div style={{ padding: "14px 0", borderBottom: "1px solid var(--border)", background: "rgba(246,243,238,.9)", backdropFilter: "blur(12px)", position: "sticky", top: 60, zIndex: 40 }}>
        <div className="cats">{CATS.map(c => <button key={c} className={`cat${cat === c ? " on" : ""}`} onClick={() => onCat(c)}>{c}</button>)}</div>
      </div>
      {products.length === 0
        ...
        : <div className="grid">{products.map((p, i) => <ProductCard key={p.id} p={p} onClick={onProduct} idx={i} />)}</div>
      }
  ...
  return (
    <div style={{ height: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "var(--bg)" }}>
      <div className="sc" style={{ background: "var(--surface)", padding: 40, width: "90%", maxWidth: 340, borderRadius: 4, boxShadow: "var(--shadow-lg)" }}>
        ...
        <h2 style={{ fontFamily: "var(--serif)", fontSize: 30, fontWeight: 300, textAlign: "center", marginBottom: 30 }}>ATELIER</h2>
        ...
        <p style={{ textAlign: "center", fontSize: 11, color: "var(--muted)", marginTop: 24 }}>Default: <code style={{ background: "var(--bg)", padding: "1px 6px", borderRadius: 2 }}>{DEF_PWD}</code></p>
      </div>
    </div>
  );
}
...
function AdminForm({ product, onCreate, onUpdate, onDone }) {
  const [form, setForm] = useState({ title: product?.title||"", price: product?.price||"", category: product?.category||CATS[1], description: product?.description||"" });
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 14 }}><label className="lbl">Categoria</label><select className="field" value={form.category} onChange={upd("category")}>{CATS.filter(c => c !== "Tutti").map(c => <option key={c} value={c}>{c}</option>)}</select></div>
    ...
    <div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div style={{ background: "var(--bg)", padding: "14px 16px", borderRadius: 4 }}>
          <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: ".08em", textTransform: "uppercase", marginBottom: 8, color: "var(--muted)" }}>💡 Variabili Template EmailJS</p>
          <p style={{ fontSize: 11, color: "var(--muted)", lineHeight: 1.9, fontFamily: "monospace" }}>{"{{to_email}} {{ordine_id}} {{data}}"}<br/>{"{{nome}} {{email_cliente}} {{telefono}}"}<br/>{"{{indirizzo}} {{prodotti}} {{totale}}"}<br/>{"{{pagamento}} {{note}}"}</p>
        </div>
        <div style={{ background: "var(--bg)", padding: "14px 16px", borderRadius: 4 }}>
...
function AdminView({ authed, onAuth, products, orders, config, tab, setTab, onCreate, onUpdate, onDelete, onToggleSold, onSaveConfig, onClose, editing, setEditing }) {
  if (!authed) return <AdminLogin password={config.adminPassword || DEF_PWD} onAuth={onAuth} onClose={onClose} />;
  const tabs = [{ id:"list",label:"Prodotti" },{ id:"form",label:editing?"Modifica":"Nuovo" },{ id:"orders",label:orders.length?`Ordini (${orders.length})`:"Ordini" },{ id:"settings",label:"Config" }];
  return (
    <div style={{ background: "var(--bg)", minHeight: "100vh" }}>
      ...
      <div style={{ padding: 20, maxWidth: 600, margin: "0 auto" }}>
        ...
        {tab==="form"     && <AdminForm key={editing?.id||"new"} product={editing} onCreate={onCreate} onUpdate={onUpdate} onDone={() => { setTab("list"); setEditing(null); }} />}
...
export default function App() {
  ...
  useEffect(() => {
    const p = db.get("products",[]); const o = db.get("orders",[]); const c = db.get("config",{});
    setProducts(p); setOrders(o); setConfig(c); setLoading(false);
  }, []);
  ...
  const saveProds = p => { setProducts(p); db.set("products",p); };
  ...
  const filtered=products.filter(p=>cat==="Tutti"||p.category===cat);
  const paged=filtered.slice(0,page*PER);
  const hasMore=paged.length<filtered.length;

  if (loading) return (<><style dangerouslySetInnerHTML={{__html:CSS}}/><div style={{height:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:22,background:"var(--bg)"}}><span style={{fontFamily:"Cormorant Garamond,Georgia,serif",fontSize:28,fontWeight:300,letterSpacing:".32em"}}>ATELIER</span><Spin size={20}/></div></>);

  return (
    <>
      <style dangerouslySetInnerHTML={{__html:CSS}}/>
      {view==="shop" && <ShopView products={paged} cat={cat} onCat={c=>{setCat(c);setPage(1);}} onProduct={p=>{setSelected(p);setView("product");window.scrollTo(0,0);}} cartCount={cart.length} onCart={()=>setShowCart(true)} onLogo={handleLogo} onMore={()=>setPage(p=>p+1)} hasMore={hasMore}/>}
      ...
      {view==="admin" && <AdminView authed={authed} onAuth={()=>setAuthed(true)} products={products} orders={orders} config={config} tab={adminTab} setTab={setAdminTab} onCreate={onCreate} onUpdate={onUpdate} onDelete={onDelete} onToggleSold={onToggleSold} onSaveConfig={saveCfg} onClose={()=>setView("shop")} editing={editing} setEditing={setEditing}/>}
